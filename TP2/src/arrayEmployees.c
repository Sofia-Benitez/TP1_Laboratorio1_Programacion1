...
int initEmployees(sEmployee list[], int len)
{
    ...
    for(int i=0; i<len; i++)//recorro el array para inicializar
    {
           list[i].isEmpty=1;
    }
...
///addEmployees
int addEmployee(sEmployee list[], int len, int* id)///devuelve 0 si esta todo ok y -1 si no
{
    int todoOk=0;
    int index;


    sEmployee newEmployee;  ///auxiliar
    ...
    if(list!=NULL && len>0)
    {
        ...
        else
        {
            ...
            newEmployee.id= *id; ///traigo el id
            ...
            newEmployee.isEmpty=0;///ahora es una estructura cargada

            list[index]=newEmployee;///copio el contenido al array

            ///actualizo el proximo id
            *id=(*id)+1;   ///referencia al nextId del main e incrementacion
...
///findEmployeeById
int findEmployeeById(sEmployee list[], int len, int id)///busqueda de un empleado por su id que me pasan por parametro (en las estructuras que estan llenas)
{
    int index=-1;///si no lo encunetra devuelve -1 que es imposible como indice

    if(list!=NULL && len>0)
    {
        for(int i=0; i<len; i++)
        {
            if(!list[i].isEmpty && list[i].id == id)//si no es 1, es decir si esta ocupado && si el indice es igual al legajo que me pasan
            {
                index=i;///guardo en la variable indice le indice de la estructura que coincide
                break;///si no rompe me devuelve el indice de la ultima estructura vacia y yo necesito el primero.
            }
        }
    }
...
{
    int todoOk=0;
    int index;//para guardar la ubicacion
    char confirma;
...
int printEmployees(sEmployee list[], int len)
{
    ...
    if(list!=NULL && len>0)
    {
        ...
        for(int i=0; i<len; i++)
        {
            ...
            {
                printEmployee(list[i], len);//llamo a la funcion que muestra un empleado.
                flag=0;
            }
        }
        if(flag)
...
///sortEmployees
int sortEmployees(sEmployee list[], int len, int order)//criterio 1: apellido A-Z y sector Asc, 2: apellido Z-A y sector Des
{
    int todoOk=0;//si no se puede ordenar
    sEmployee auxEmpleado;//auxiliar de lo que voy a ordenar

    if(list!=NULL && len>0 && order>=1 && order <=2)//validacion
    {
       if(order==1)
       {
            ...
            for(int i=0; i<len-1; i++)
                {
                    ...
                    {
                        if((strcmp(list[i].lastName, list[j].lastName)>0) || ((strcmp(list[i].lastName, list[j].lastName), 0) && list[i].sector >list[j].sector))//si se da los elementos estan desordenados
                        {
                            auxEmpleado=list[i];
                            list[i]=list[j];
                            list[j]=auxEmpleado;
           ...
           for(int i=0; i<len-1; i++)
            {
                for(int j=i+1; j<len; j++)
                {
                    if((strcmp(list[j].lastName, list[i].lastName)>0) || ((strcmp(list[j].lastName, list[i].lastName),0) && list[i].sector<list[j].sector))//si se da los elementos estan desordenados
                    {
                        auxEmpleado=list[i];
                        list[i]=list[j];
                        list[j]=auxEmpleado;
...
int sueldosTotalesYPromedio(sEmployee list[], int len)
{
    ...
    if(list!=NULL && len>0)
    {
        ...
        for(int i=0; i<len; i++)
        {
            ...
            {
                printf("%s %s \n", list[i].name, list[i].lastName);//imprimo ls nombres de los empleados cuyos sueldos son mayors que el promedio
            }
        }
